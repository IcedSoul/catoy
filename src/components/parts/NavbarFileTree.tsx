...
export const NavbarFileTree = ({opened, setOpened, type}: NavbarFileTreeProps) => {
    ...
    useEffect(() => {
        ...
        root.uri = type === "code" ? "projects" : "notes"
        setTree({...root})
    },[])
    ...
    const createNewDirectoryOrFile = (fileType: "directory" | "file", treeNode: TreeNode<any, string>, event: any) => {
        event.stopPropagation()
        if(!treeNode.children){
            treeNode.children = []
        }
        const fileName = type === "code" ? "NewFile.ts" : "NewNote.note"
        treeNode.children.push({
            id: treeNode.id,
            type: fileType,
            uri: `${treeNode.uri}/${fileType === "directory" ? "NewFolder" : fileName}`,
            expanded: false,
            children: [],
            edit: true,
            parent: treeNode
    ...
    const confirmCreate = (treeNode: TreeNode<any, string>) => {
        ...
        if(treeNode.type === "file"){
            type === "code" && createCode(treeNode).then(() => {
                createOrUpdateFileTree(treeNode)
            })
            type === "note" && createNote(treeNode).then(() => {
                createOrUpdateFileTree(treeNode)
            })
        } else {
            createOrUpdateFileTree(treeNode)
    ...
    const loadFileTree = () => {
        fetch("/api/fileTree?".concat(new URLSearchParams({ type }).toString()))
    ...
    const createOrUpdateFileTree
        ...
        fetch("/api/fileTree", requestInit).then(() => {
            loadFileTree()
        }).catch((error) => {
            notifications.show(error)
        })
    ...
    const deleteTreeNode = (treeNode: TreeNode<any, string>) => {
        if(treeNode.parent && "root" in treeNode.parent){
            ...
            if(treeNode.type === "file"){
                type === "code" && deleteCode(treeNode).then()
                type === "note" && deleteNote(treeNode).then()
                if(refreshCode){
                    setOpened(false)
    ...
    const deleteFileTree = (id: string) => {
        ...
        const requestInit: RequestInit = {
            ...
            headers,
            body: JSON.stringify({id, classification: type})
        }
        fetch("/api/fileTree", requestInit).then(() => {
            loadFileTree()
        }).catch((error) => {
